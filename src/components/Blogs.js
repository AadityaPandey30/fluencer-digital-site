...
const Blogs = () => {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        const response = await axios.get("https://fluencerblogbackend.onrender.com/api/blogs");
      ...
      <div className="blogs grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-6xl mx-auto px-4">
        {blogs.length > 0 ? (
          blogs.map((blog) => (
            ...
            >
              <img src={`https://fluencerblogbackend.onrender.com${blog.image}`} className=" w-full h-2/3 max-h-[300px] rounded-md" alt="Fluencer Blogs"/>
